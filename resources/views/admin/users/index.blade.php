<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-wrap items-center justify-between gap-4">
            ...
            <a href="{{ route('admin.users.create') }}" class="inline-flex items-center px-4 py-2 bg-blue-900 dark:bg-blue-950 border border-blue-950 dark:border-blue-900 rounded-md font-semibold text-xs text-amber-50 uppercase tracking-widest hover:bg-blue-800 dark:hover:bg-blue-900 shadow-sm">
                Novo usuário
            </a>
        </div>
    </x-slot>
                                ...
                                <tbody>
                                    @foreach ($users as $u)
                                        ...
                                        <tr class="border-b border-blue-50 dark:border-slate-700 last:border-0">
                                            <td class="py-3 pe-4 font-medium text-slate-900 dark:text-slate-50">{{ $u->name }}</td>
                                            <td class="py-3 pe-4 text-slate-700 dark:text-slate-300">{{ $u->email }}</td>
                                            <td class="py-3 pe-4 text-slate-700 dark:text-slate-300">{{ $u->phone ?? '-' }}</td>
                                            <td class="py-3 pe-4 text-slate-700 dark:text-slate-300">{{ $count }}</td>
                                            <td class="py-3">
